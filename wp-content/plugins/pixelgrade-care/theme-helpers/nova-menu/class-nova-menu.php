...
class Pixelgrade_Nova_Menu {
	...
	public function register_hooks() {
		add_action( 'after_setup_theme', array( $this, 'add_shortcodes' ), 25 );
	...
	public function nova_menu_shortcode_html( $atts ) {
		...
		if ( $query->have_posts() ) { ?>
				...
				while ( $query->have_posts() ) {
                    ...
                    if ( $has_highlight_title ) {
						...
						<div class="menu-list__item-meta">
							...
							<?php edit_post_link( esc_html__( 'Edit', 'pixelgrade_care' ), '<span class="edit-link">', '</span>' ); ?>
						</div><!-- .entry-meta -->
			...
			<p class="jetpack-nova-menu-shortcode no-items"><?php esc_html_e( 'You seem to be short on menu entries. You can start creating them on your dashboard.', 'pixelgrade_care' ); ?></p>
			<?php
		}
		$html = ob_get_clean();
	...
	public function the_menu_item_labels( $post_id, $exclude_labels = array() ) {
		...
		$html = '<div class="menu-item-labels"><span class="label-text screen-reader-text">' . esc_html__( 'Labels', 'pixelgrade_care' ) . ':</span>';
		$labels = array();
		// Loop through all the labels
		foreach ( $menu_item_labels as $menu_item_label ) {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'pixelgrade_care' ), esc_html( $this->_version ) );
	} // End __clone ()
