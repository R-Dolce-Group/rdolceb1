...
class Pixelgrade_Jetpack_Portfolio_Shortcode {
	...
	static function portfolio_shortcode_html( $atts ) {
		...
		if ( ! file_exists( $template ) ) {
			_doing_it_wrong( __FUNCTION__, sprintf( __( '%s does not exist.', 'pixelgrade_care' ), '<code>' . $template . '</code>' ), '1.2.6' );
		} else {
			include( $template );
		}
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'pixelgrade_care' ), '1.0.0' );
	} // End __clone ()
