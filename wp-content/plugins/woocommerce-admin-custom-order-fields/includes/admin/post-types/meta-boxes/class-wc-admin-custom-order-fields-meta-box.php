...
class WC_Admin_Custom_Order_Fields_Meta_Box {
	...
	public function save( $order_id, $post ) {
		...
		foreach ( $order_fields as $custom_field ) {
			...
			if ( '0' === $updated_value || ! empty( $updated_value ) ) {
				...
				if ( 'date' === $order_fields[ $field_id ]->get_type() ) {
					...
					SV_WC_Order_Compatibility::update_meta_data( $order, $field_meta_key, $order_fields[ $field_id ]->get_value() );

					// This column is used so that date fields can be searchable.
					SV_WC_Order_Compatibility::update_meta_data( $order, $field_meta_key . '_formatted', $order_fields[ $field_id ]->get_value_formatted() );

				} else {

					SV_WC_Order_Compatibility::update_meta_data( $order, $field_meta_key, $updated_value );
				}

			// ...Or if it's empty, delete the custom field meta altogether.
			} else {

				SV_WC_Order_Compatibility::delete_meta_data( $order, $field_meta_key );
				SV_WC_Order_Compatibility::delete_meta_data( $order, $field_meta_key . '_formatted' );
			}
		}
	}
