...
class WC_Admin_Custom_Order_Fields_Shop_Order_CPT {
	...
	public function restrict_orders() {
		...
		<?php foreach ( wc_admin_custom_order_fields()->get_order_fields() as $order_field ) : ?>

			<?php if ( $order_field->is_filterable() ) : ?>

				<?php if ( $order_field->type === 'date' ) : ?>

					<?php // filterable date field: provide a monthly dropdown ?>
					<?php $this->render_months_dropdown( $order_field->label, $order_field->get_meta_key() ); ?>

				<?php elseif ( $order_field->has_options() ) : ?>

					<?php // filterable multi item field (select, multiselect, radio, checkbox), provide a dropdown ?>
				...
				<?php elseif ( $order_field->type === 'text' ) : ?>

					<?php if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) : ?>

						<select
							class="sv-wc-enhanced-search"
							name="<?php echo esc_attr( $order_field->get_meta_key() ); ?>" style="min-width:200px;"
							...
							data-request_data="<?php echo esc_attr( json_encode( array(
