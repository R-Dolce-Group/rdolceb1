...
class Submit_Claim_Form extends \WP_Job_Manager_Form{
	...
	public function login_register_view(){
		wp_enqueue_script( 'wpjmcl_pass-meter' );
...
?>
		...
		<form id="<?php echo esc_attr( $this->get_form_name() ); ?>" class="job-manager-form wpjmcl_form wpjmcl_form_login_register" method="post">
			...
			<?php if ( job_manager_enable_registration() ) { ?>
				<p>
...
<?php
	...
	public function login_register_handler(){
		...
		elseif( job_manager_enable_registration() && isset( $this->form_data['submit'] ) && isset( $this->form_data['_wpjmcl_register_nonce'] ) ){
			...
			if( wp_verify_nonce( $this->form_data['_wpjmcl_register_nonce'], __FILE__ ) ){
				...
				if ( job_manager_user_requires_account() ) {
					if ( ! job_manager_generate_username_from_email() && empty( $_POST['create_account_username'] ) ) {
						$this->add_error( __( 'Please enter a username.', 'wp-job-manager-claim-listing' ) );
					}
					if ( empty( $_POST['create_account_email'] ) ) {
						$this->add_error( __( 'Please enter your email address.', 'wp-job-manager-claim-listing' ) );
					}
				}
				/**
				...
				if ( ! empty( $_POST['create_account_email'] ) ) {
					...
					$enable_password = get_option( 'wpjmcl_enable_registration_password', 1 );
					if ( $enable_password ) {
						$account_data['password'] = trim( $_POST['password'] );
					}
					$create_account = wp_job_manager_create_account( $account_data );
				}
		...
		/* Initial View. */
			...
			$this->add_error( __( 'You must sign in to claim a listing.', 'wp-job-manager-claim-listing' ) );
		}
	...
	public function claim_listing_view(){
		...
		<form id="<?php echo esc_attr( $this->get_form_name() ); ?>" class="job-manager-form wpjmcl_form wpjmcl_form_claim_listing" method="post">
			...
			<?php $this->signin_field(); // display logout link ?>
			<?php $this->listing_field(); // display listing info ?>
	...
	public function claim_listing_handler(){
		...
		/* User not logged in, go to previous step. */
		if ( ! is_user_logged_in() ) {
			$this->previous_step();
			return;
		}
		...
		elseif( isset( $_POST['submit'] ) && isset( $_POST['_wpjmcl_submit_claim_nonce'] ) && wp_verify_nonce( $_POST['_wpjmcl_submit_claim_nonce'], __FILE__ ) ){
	...
	public function claim_detail_view(){
		...
		<div class="job-manager-message">
			<?php printf( __( 'Your claim status for %s is %s.', 'wp-job-manager-claim-listing' ), $listing_link, '<strong>' . $claim_status . '</strong>' );?>
		</div>
	...
	public function signin_field(){
		...
		/* Add Password Field */
		add_action( 'job_manager_register_form', function() {
			$enable_password = get_option( 'wpjmcl_enable_registration_password', 1 );
			if ( ! $enable_password ) {
				return;
			}
?>
<fieldset>
	<label for="account_password"><?php _e( 'Password', 'wp-job-manager-claim-listing' ); ?> <small><?php _e( '(required)', 'wp-job-manager-claim-listing' ); ?></small></label>
	<div class="field">
		<input required type="password" class="input-text" name="password" id="account_password" value="<?php echo empty( $_POST['password'] ) ? '' : esc_attr( sanitize_text_field( trim( $_POST['password'] ) ) ); ?>" />
