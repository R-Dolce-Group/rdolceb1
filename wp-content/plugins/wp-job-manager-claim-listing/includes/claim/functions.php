...
final class Functions{
	 ...
	 * @since 3.0.0
	 */
	public static function create_new_claim( $listing_id, $user_id, $claim_data = '' ){
		...
		if ( !is_wp_error( $claim_id ) ) {
			...
			do_action( 'wpjmcl_create_new_claim', $claim_id, $context = 'front' );
	...
	public static function get_data( $claim_id ){
		...
		$listing_obj = get_post( $listing_id );
		if( !$claim_obj || !$claimer_obj || !$listing_obj ) return false;
		...
		$datas = array(
			...
			'claimer_name'     => $claimer_obj->data->display_name,
			'claimer_login'    => $claimer_obj->data->user_login,
			'claimer_email'    => $claimer_obj->data->user_email,
			/* Listing */
			'listing_id'       => $listing_id,
