...
final class Order_Setup{
	...
	function order_created( $order_id ){
		...
		foreach ( $order->get_items() as $item ) {
			...
			if ( $product->is_type( array( 'job_package', 'job_package_subscription' ) ) && $order->customer_user && isset( $item['claim_id'] ) ) {
			...
			if ( $product->is_type( array( 'job_package', 'job_package_subscription' ) ) && $order->customer_user ) {
				...
				if( isset( $item['job_id'] ) ){
					...
					if ( $user_package ) {
						...
						wc_paid_listings_increase_package_count( $order->customer_user, $user_package->id );
						update_post_meta( $job->ID, '_user_package_id', $user_package->id );

						if ( $product->is_type( 'job_package_subscription' ) ) {
							do_action( 'wc_paid_listings_switched_subscription', $job->ID, $user_package );
						}
					}
				}
			}
