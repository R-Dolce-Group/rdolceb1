...
<div class="job-manager-uploaded-file">
	...
	if ( is_numeric( $value ) ) {
		...
		$thumbnail_src = $thumbnail_src ? $thumbnail_src[0] : '';
		$image_src = wp_get_attachment_image_src( absint( $value ), 'full' );
		$image_src = $image_src ? $image_src[0] : '';
	} else {
		$image_src = $thumbnail_src = $value;
	}
	$extension = ! empty( $extension ) ? $extension : substr( strrchr( $image_src, '.' ), 1 );
	if ( 'image' === wp_ext2type( $extension ) ) : ?>
		<span class="job-manager-uploaded-file-preview"><img src="<?php echo esc_url( $thumbnail_src ); ?>" /> <a class="job-manager-remove-uploaded-file" href="#">[<?php _e( 'remove', 'wp-job-manager' ); ?>]</a></span>
	<?php else : ?>
		<span class="job-manager-uploaded-file-name"><code><?php echo esc_html( basename( $thumbnail_src ) ); ?></code> <a class="job-manager-remove-uploaded-file" href="#">[<?php _e( 'remove', 'wp-job-manager' ); ?>]</a></span>
	<?php endif; ?>
