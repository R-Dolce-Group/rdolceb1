...
if ( ! class_exists( 'Jetpack_Portfolio' ) ) {
	...
	class Jetpack_Portfolio {
		...
		function settings_api_init() {
			add_settings_field(
				self::OPTION_NAME,
				'<span class="cpt-options">' . __( 'Portfolio Projects', 'pixelgrade_care' ) . '</span>',
		...
		function setting_html() {
			if ( current_theme_supports( self::CUSTOM_POST_TYPE ) ) : ?>
				<p><?php printf( __( 'Your theme supports <strong>%s</strong>', 'pixelgrade_care' ), self::CUSTOM_POST_TYPE ); ?></p>
			<?php else : ?>
				<label for="<?php echo esc_attr( self::OPTION_NAME ); ?>">
					...
					<a target="_blank"
					   href="http://en.support.wordpress.com/portfolios/"><?php esc_html_e( 'Learn More', 'pixelgrade_care' ); ?></a>
				</label>
			<?php endif;
			if ( get_option( self::OPTION_NAME, '0' ) || current_theme_supports( self::CUSTOM_POST_TYPE ) ) :
		...
		function register_post_types() {
			...
			register_post_type( self::CUSTOM_POST_TYPE, array(
				'description'     => __( 'Portfolio Items', 'pixelgrade_care' ),
				'labels'          => array(
		...
		function customize_register( $wp_customize ) {
			...
			$wp_customize->add_section( 'jetpack_portfolio', array(
				'title'          => esc_html__( 'Portfolio', 'pixelgrade_care' ),
			...
			if ( isset( $options[0]['title'] ) && true === $options[0]['title'] ) {
				$wp_customize->add_setting( 'jetpack_portfolio_title', array(
					'default'              => esc_html__( 'Projects', 'pixelgrade_care' ),
					'type'                 => 'option',
				...
				$wp_customize->add_control( 'jetpack_portfolio_title', array(
					...
					'label'   => esc_html__( 'Portfolio Archive Title', 'pixelgrade_care' ),
					'type'    => 'text',
				) );
			}
			...
			if ( isset( $options[0]['content'] ) && true === $options[0]['content'] ) {
				...
				$wp_customize->add_control( 'jetpack_portfolio_content', array(
					'section' => 'jetpack_portfolio',
					'label'   => esc_html__( 'Portfolio Archive Content', 'pixelgrade_care' ),
					'type'    => 'textarea',
				) );
			}
			...
			if ( isset( $options[0]['featured-image'] ) && true === $options[0]['featured-image'] ) {
				...
				$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'jetpack_portfolio_featured_image', array(
					'section' => 'jetpack_portfolio',
					'label'   => esc_html__( 'Portfolio Archive Featured Image', 'pixelgrade_care' ),
				) ) );
			}
		}
		...
		static function portfolio_shortcode_html( $atts ) {
			...
			if ( $query->have_posts() ) {
				...
				<p>
					<em><?php _e( 'Your Portfolio Archive currently has no entries. You can start creating them on your dashboard.', 'pixelgrade_care' ); ?>
				</p></em>
				<?php
			}
			$html = ob_get_clean();
