 ...
 * Description: Adds Address Validation and Postcode Lookup to WooCommerce via PCA Predict, SmartyStreets, and more!
 * Author: SkyVerge
...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.6.0', __( 'WooCommerce Address Validation', 'woocommerce-address-validation' ), __FILE__, 'init_woocommerce_address_validation', array(
...
class WC_Address_Validation extends SV_WC_Plugin {
	...
	public function __construct() {
		...
		parent::__construct(
			self::PLUGIN_ID,
			self::VERSION,
			array( 'text_domain' => 'woocommerce-address-validation' )
		);
	...
	/**
	 * Render a notice for the user to switch to Addressy after upgrade from pre 2.0.0
	 *
	 * @since 2.0.0
	...
	public function add_admin_notices() {
		...
		if ( get_option( 'wc_address_validation_encourage_addressy_upgrade_switch' ) ) {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				sprintf( '%s %s', __( "Address Validation has been upgraded! We've added support for Addressy, which can perform address lookups for all countries you sell to.", 'woocommerce-address-validation' ), sprintf( /* translators: Placeholders: %1$s, %3$s - opening <a> tag, %2$s, %4$s - closing </a> tag */ __( '%1$sAdjust your settings%2$s or %3$slearn more about Addressy%4$s.', 'woocommerce-address-validation' ), '<a href="' . $this->get_settings_url() . '">', '</a>', '<a href="https://www.addressy.com/integrations/woocommerce-address-verification/ADRSY11126">', '</a>' ) ),
	...
	public function get_support_url() {
		return 'http://woocommerce.com/my-account/tickets/';
	}
