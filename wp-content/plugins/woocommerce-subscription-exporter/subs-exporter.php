...
Tested up to: 3.9.1 & WooCommerce 2.1.12 & WooCommerce Subscriptions 1.5.6
...
if ( is_admin() ) {
	...
	function wc_subs_exporter_admin_init() {
		...
		switch( $action ) {
			...
			case 'export':
				...
				if ( isset( $_POST['processed_step'] ) && 'found_subscription_keys' == $_POST['processed_step'] && isset( $_POST['hash'] ) ) {
					// finish processing an ongoing export action
					$hash   = $_POST['hash'];
					$export = get_transient( wc_subs_exporter_get_subscriptions_transient_name( $hash ) );
					delete_transient( wc_subs_exporter_get_subscriptions_transient_name( $hash ) );
				}
	...
	function wc_subs_exporter_manage_form() {
		...
		require_once( 'templates/admin/wc-subs-exporter-admin.php' );
	}
}
?>