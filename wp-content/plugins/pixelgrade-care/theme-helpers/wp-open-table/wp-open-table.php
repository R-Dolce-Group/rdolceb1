...
class Open_Table_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'open_table_widget',
			esc_html__( 'OpenTable Widget', 'pixelgrade_care' ),
			array(
				'description' => __( 'OpenTable Widget for WordPress', 'pixelgrade_care' ),
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $open_table_embed_code ) ) {
			...
			printf( __( '<p>You first need to copy&paste your EMBED code from <a href="%s">OpenTable Reservations</a> in <a href="%s">Appearance > Customize > General</a>.</p>', 'pixelgrade_care' ),
	...
	?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title:', 'pixelgrade_care' ); ?>
			</label>
			<?php
			printf( '<input class="widefat" id="%1$s" name="%2$s" type="text" value="%3$s" />',
				esc_attr( $this->get_field_id( 'title' ) ),
				esc_attr( $this->get_field_name( 'title' ) ),
				isset( $instance['title'] ) ? esc_attr( $instance['title'] ) : esc_attr__( 'My OpenTable Widget', 'pixelgrade_care' )
			);
			?>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'widget_style' ) ); ?>"><?php esc_html_e( 'Widget Style', 'pixelgrade_care' ); ?>:</label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'widget_style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'widget_style' ) ); ?>" class="widefat">
				<?php
				$options = array(
					'standard' => esc_html__( 'Standard', 'pixelgrade_care' ),
					'tall' => esc_html__( 'Tall', 'pixelgrade_care' ),
					'wide' => esc_html__( 'Wide', 'pixelgrade_care' ),
					'button' => esc_html__( 'Button', 'pixelgrade_care' ),
				);
...
function pixelgrade_open_table_customify_option( $options ) {
	...
	$options['general']['options']['open_table_embed_code'] = array(
		'type'    => 'textarea',
		'label'   => esc_html__( 'OpenTable Embed Code', 'pixelgrade_care' ),
		'default' => '',
		'desc'    => sprintf( __( 'Paste your EMBED code from <a href="%s">OpenTable Reservations</a>.', 'pixelgrade_care' ),
			'https://www.otrestaurant.com/marketing/ReservationWidget" target="_blank'
		),
		'priority' => 30,
	);
...
function pixelgrade_ot_reservation_widget_shortcode( $atts ) {
	...
	if ( ! empty( $open_table_embed_code ) ) {
		// we need to process it a bit - extract the parameters
		$params = Open_Table_Widget::extract_embed_params( $open_table_embed_code );
		...
		// Now rebuild the parameters with out special alterations depending on widget options
		return '<script type="text/javascript" src="' . esc_attr( Open_Table_Widget::build_embed_url( $params ) ) . '"></script>';
	} else {
		return sprintf( __( '<p>You first need to copy&paste your EMBED code from <a href="%s">OpenTable Reservations</a> in <a href="%s">Appearance > Customize > General</a>.</p>', 'pixelgrade_care' ),
			'https://www.otrestaurant.com/marketing/ReservationWidget" target="_blank',
			esc_url( add_query_arg(
				array(
					array( 'autofocus' => array( 'section' => 'pixelgrade_options[general]' ) ),
