...
function wppb_msf_scripts() {
    ...
    if( defined( 'PROFILE_BUILDER_VERSION' ) && ( $screen->id == 'profile-builder_page_manage-fields' || $screen->id == 'wppb-rf-cpt' || $screen->id == 'wppb-epf-cpt' ) ) {
        ...
        $vars_array = array(
            ...
            'tabTitle'		            => __( 'Step', 'profile-builder-multi-step-forms-add-on' ),
            'tabTitlePlaceholder'	    => __( 'Title for Tab', 'profile-builder-multi-step-forms-add-on' ),
            'tabsTitleDesc'		        => __( 'Add Break Points to edit tabs title.', 'profile-builder-multi-step-forms-add-on' ),
            'tabsTitleDescUnsavedForm'  => __( 'Publish the form to edit tabs title.', 'profile-builder-multi-step-forms-add-on' ),
            'alertUnsavedForm'          => __( 'Publish the form before adding Break Points!', 'profile-builder-multi-step-forms-add-on' ),
            'alertAjaxRequestInProcess' => __( 'Request in process, please wait a few seconds before a new one!', 'profile-builder-multi-step-forms-add-on' )
        );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_frontend_scripts() {
    global $wppb_shortcode_on_front;

    if( $wppb_shortcode_on_front ) {
        wp_enqueue_script( 'wppb-msf-script-frontend', plugin_dir_url( __FILE__ ) . 'assets/js/frontend-multi-step-forms.js', array( 'jquery' ) );
...
add_action( 'wp_footer', 'wppb_msf_frontend_scripts' );

/**
 * Function that enqueues the necessary styles in frontend
 *
 * @since   v.1.0.0
 */
function wppb_msf_frontend_style() {
    wp_enqueue_style( 'wppb-msf-style-frontend', plugin_dir_url( __FILE__ ) . 'assets/css/frontend-multi-step-forms.css' );
}
add_action( 'wp_enqueue_scripts', 'wppb_msf_frontend_style' );
...
function wppb_msf_add_meta_boxes() {
    add_meta_box( 'wppb-msf-side', __( 'Multi-Step Forms', 'profile-builder-multi-step-forms-add-on' ), 'wppb_msf_meta_boxes_content', array( 'wppb-epf-cpt', 'wppb-rf-cpt', 'profile-builder_page_manage-fields' ), 'side', 'low' );
}
...
function wppb_msf_meta_boxes_content( $post ) {
    ...
    if( $screen->id == 'profile-builder_page_manage-fields' ) {
        ...
        if( isset( $_POST['wppb_msf_save_options'] ) && wp_verify_nonce( $_POST['wppb_msf_save_options'], 'wppb-msf-options-verify' ) ) {
            ...
            echo '<div class="notice notice-success is-dismissible"><p>'. __( 'Multi-Step Forms options updated.', 'profile-builder-multi-step-forms-add-on' ) .'</p></div>';
        }

        ?>
        <form id="wppb-msf-options" name="wppb-msf-options" method="POST" action="">
            <p><strong><?php _e( 'Enable on:', 'profile-builder-multi-step-forms-add-on' ); ?></strong></p>
            <p><label><input type="checkbox" id="pb-default-register" name="pb-default-register" value="yes" <?php echo $wppb_msf_options != 'not_found' && $wppb_msf_options['pb-default-register'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'PB Default Register Form', 'profile-builder-multi-step-forms-add-on' ); ?></label></p>
            <p><label><input type="checkbox" id="pb-default-edit-profile" name="pb-default-edit-profile" value="yes" <?php echo $wppb_msf_options != 'not_found' && $wppb_msf_options['pb-default-edit-profile'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'PB Default Edit Profile Form', 'profile-builder-multi-step-forms-add-on' ); ?></label></p>
            <p class="description"><?php _e( 'To enable it on Multiple Registration and Edit-Profile Forms you must add Break Points in each form page.', 'profile-builder-multi-step-forms-add-on' ); ?></p>
            <p><strong><?php _e( 'Pagination and Tabs:', 'profile-builder-multi-step-forms-add-on' ); ?></strong></p>
            <p><label><input type="checkbox" id="msf-pagination" name="msf-pagination" value="yes" <?php echo $wppb_msf_options != 'not_found' && isset( $wppb_msf_options['msf-pagination'] ) && $wppb_msf_options['msf-pagination'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Pagination', 'profile-builder-multi-step-forms-add-on' ); ?></label></p>
            <p><label><input type="checkbox" id="msf-tabs" name="msf-tabs" value="yes" <?php echo $wppb_msf_options != 'not_found' && isset( $wppb_msf_options['msf-tabs'] ) && $wppb_msf_options['msf-tabs'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Tabs', 'profile-builder-multi-step-forms-add-on' ); ?> <span class="wppb-msf-edit-tabs-title" style="display: none" >- <a id="wppb-msf-edit-tabs-title" href="#"><?php _e( 'Edit Tabs Title', 'profile-builder-multi-step-forms-add-on' ); ?></a></span></label></p>
            <div class="wppb-msf-tabs-title-container" style="display: none;"></div>
            <div id="major-publishing-actions"><input name="save" type="submit" class="button button-primary button-large" id="wppb_msf_submit" value="<?php _e( 'Update Multi-Step', 'profile-builder-multi-step-forms-add-on' ); ?>"></div>
        ...
        if( get_post_status( $post->ID ) !== 'publish' ) {
            $wppb_msf_check_post_status = '<p class="description wppb-msf-post-publish-error">'. __( 'Publish the form before adding Break Points!', 'profile-builder-multi-step-forms-add-on' ) .'</p>';
        }

        ?>
        <?php echo $wppb_msf_check_post_status; ?>
        <p class="description"><?php _e( 'To enable MSF you must add Break Points.', 'profile-builder-multi-step-forms-add-on' ); ?></p>
        <p><strong><?php _e( 'Pagination and Tabs:', 'profile-builder-multi-step-forms-add-on' ); ?></strong></p>
        <p><label><input type="checkbox" id="msf-pagination" name="msf-pagination" value="yes" <?php echo $wppb_msf_post_options != 'not_found' && $wppb_msf_post_options['msf-pagination'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Pagination', 'profile-builder-multi-step-forms-add-on' ); ?></label></p>
        <p><label><input type="checkbox" id="msf-tabs" name="msf-tabs" value="yes" <?php echo $wppb_msf_post_options != 'not_found' && $wppb_msf_post_options['msf-tabs'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Tabs', 'profile-builder-multi-step-forms-add-on' ); ?> <span class="wppb-msf-edit-tabs-title" style="display: none" >- <a id="wppb-msf-edit-tabs-title" href="#"><?php _e( 'Edit Tabs Title', 'profile-builder-multi-step-forms-add-on' ); ?></a></span></label></p>
        <div class="wppb-msf-tabs-title-container" style="display: none;"></div>
        <?php
...
function wppb_msf_after_form_fields( $output, $form_type, $form_id ) {
    $next_button_label = apply_filters( 'wppb_msf_next_button_label', __( 'Next', 'profile-builder-multi-step-forms-add-on' ) );
    $previous_button_label = apply_filters( 'wppb_msf_previous_button_label', __( 'Previous', 'profile-builder-multi-step-forms-add-on' ) );
        ...
        if( isset( $wppb_msf_options ) && $wppb_msf_options != 'not_found' && $wppb_msf_options['msf-tabs'] == 'yes' ) {
            ...
            for( $i = 0; $i <= count( $break_points ); $i++ ) {
                if( isset( $wppb_msf_tab_titles ) && $wppb_msf_tab_titles != 'not_found' && isset( $wppb_msf_tab_titles[$i] ) ) {
                    ...
                    $tab_title = __( 'Step', 'profile-builder-multi-step-forms-add-on' ) .' '. ( $i + 1 );
                }
 ...
 * @since   v.1.0.0
 */
function wppb_msf_delete_break_points( $post_meta, $post_id, $element_id ) {
    if( isset( $element_id ) ) {
 ...
 * @return null
 */
function wppb_msf_init_translation() {
    $current_theme = wp_get_theme();
    if( ! empty( $current_theme->stylesheet ) && file_exists( get_theme_root() . '/' . $current_theme->stylesheet . '/local_pb_lang' ) )
        load_plugin_textdomain( 'profile-builder-multi-step-forms-add-on', false, basename( dirname( __FILE__ ) ) . '/../../themes/' . $current_theme->stylesheet . '/local_pb_lang' );
    else
        load_plugin_textdomain( 'profile-builder-multi-step-forms-add-on', false, basename( dirname( __FILE__ ) ) . '/translation/' );
}
add_action( 'init', 'wppb_msf_init_translation', 8 );
