...
final class Setup {
	...
	**/
	...
	function scripts() {
		...
		/* Only load password meter if registration password field enabled */
		$enable_password = get_option( 'wpjmcl_enable_registration_password', 1 );
		if ( $enable_password && apply_filters( 'wpjmcl_pass-meter_js', true ) ) {
			wp_register_script( 'wpjmcl_pass-meter', URI . 'assets/pass-meter.js', array( 'jquery', 'password-strength-meter' ), VERSION, true );
		}
	}
