...
final class Setup{
	...
	public function add_settings( $settings ){
		...
		/* Enable Password Field On Account Creation */
		$settings['wpjmcl_settings'][1][] = array(
			'name'      => 'wpjmcl_enable_registration_password',
			'label'     => __( 'Account Creation', 'wp-job-manager-claim-listing' ),
			'cb_label'  => __( 'Automatically generate customer password.', 'wp-job-manager-claim-listing' ),
			'desc'      => '',
			'type'      => 'checkbox',
			'std'       => 1,
		);
	...
	/**
	 * Admin Notice: If page is not set yet.
	 */
	public function admin_notice(){
		global $hook_suffix;
		$claim_page = get_option( 'job_manager_claim_listing_page_id' );
		if( ! $claim_page && ( 'job_listing_page_job-manager-settings' != $hook_suffix ) ){
			...
			<div class="notice notice-info is-dismissible">
				<p><?php printf( __( 'To complete setup of the Claim Listing plugin, please visit the <a href="%s">WP Job Manager Settings</a> panel.', 'wp-job-manager-claim-listing' ), esc_url( $url ) ); ?></p>
			</div>
			<?php
		}
	}
