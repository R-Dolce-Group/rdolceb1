...
class PixelgradeCare_StarterContent {
	...
	public function rest_upload_media( $request ) {
		...
		return rest_ensure_response( array(
			'code'    => 'error',
			'message' => 'Something went wrong with uploading the media file.',
			'data'    => array(
				'error' => $attachment_id,
			),
		) );
	...
	private function import_taxonomy( $base_url, $args ) {
		...
		if ( ! taxonomy_exists( $args['tax'] ) ) {
			return rest_ensure_response( array(
				'code'    => 'missing_tax',
				'message' => $args['tax'] . ' does not exists!',
				'data'    => array(),
			) );
		}
	...
	private function import_parsed_widgets( $base_url ) {
		...
		if ( $this->parse_import_data( $sidebar_data ) ) {
			...
			// @todo What The F**k Does This Mean? Ugly bug! What is the bug? Where the f**k is it?!?
